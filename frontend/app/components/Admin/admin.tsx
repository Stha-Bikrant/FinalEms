...
export default function AdminDashboard() {
  ...
  const [userRole, setUserRole] = useState(1); // User role as number (0 = Auditor, 1 = Guest)
  ...
  const assignCaseToUser = async () => {
    if (!caseUserAddress || !caseNumber) {
      ...
      return; // Prevent contract call if inputs are missing
    }

    if (contract) {
      try {
        const tx = await contract.assignCaseToUser(caseUserAddress, caseNumber);
        await tx.wait();
